...
#
# Usage:
#   bash tuned/gb10/raft_container_gb10.sh           # all phases: build → smoke → test → pytest
#   bash tuned/gb10/raft_container_gb10.sh --build   # build image only
#   bash tuned/gb10/raft_container_gb10.sh --smoke   # smoke test only (no GPU needed)
#   bash tuned/gb10/raft_container_gb10.sh --test    # gtest only (requires SM_121a GPU)
#   bash tuned/gb10/raft_container_gb10.sh --pytest  # Python tests via pylibraft-gb10-cu13 wheel
#
...
# source first:
#   bash tuned/gb10/raft_cupy_build_gb10.sh   # outputs cupy_cuda13x-*.whl to dist/gb10/
...
# together.  See tuned/gb10/raft_cupy_build_gb10.sh for the full explanation.
...
# tuned/devices/gb10.conf is pure variable assignment (no nvcc probing),
...
# drifting from env.sh's own value.
...
phase_test() {
    ...
    if [[ ! -f "${HOST_BUILD_DIR}/gtests/UTILS_TEST" ]] || \
       [[ ! -f "${HOST_BUILD_DIR}/gtests/LINALG_TEST" ]]; then
        ...
        echo "           Run 'bash tuned/gb10/raft_build_gb10.sh' on the host first, then re-run --test."
        return 0
    fi
...
phase_pytest() {
    ...
    if [[ -z "${LIBRAFT_WHL}" ]] || [[ -z "${PYLIBRAFT_WHL}" ]]; then
        ...
        echo "           Run 'bash tuned/gb10/raft_wheel_gb10.sh' first to build them."
        return 0
    fi
    ...
    # Prefer a source-built arch-specific wheel (tuned/gb10/raft_cupy_build_gb10.sh) if
    ...
    #   1. cupy-*.whl        — source-built SM_121a-only (tuned/gb10/raft_cupy_build_gb10.sh), ~36 MB
    ...
    docker run --rm \
        ...
        bash -c "
            ...
            pip install --quiet \
                ...
                -r /home/coder/raft/tuned/requirements-build-cuda13x.txt \
