...
# dist dir so a single `pip install dist/rtx50xx/*.whl` and a single
...
# own cross-package dependency, or libraft-rtx50xx-cu13's own metadata
...
rm -rf /tmp/raft-rtx50xx-install
embed_build_info "${LIBRAFT_STAGED}/libraft/lib64/${LIBRAFT_SONAME}" "${ARCH}" "libraft" "${VERSION}+cu${CUDA_VERSION_COMPACT}"
...
# is left untouched -- matches upstream, no RAFT_VARIANT env var or
# dynamic import_module() needed. See the PyTorch-ordering citation in
# raft_wheel_common.sh's validate_wheels() for why.
...
remove_yaml_include_or_fail "${WHEEL_SRC}/dependencies.yaml" "py_run_raft_dask" "depends_on_distributed_ucxx" \
    "drop hard ucxx dependency from raft-dask-rtx50xx (see ucx.py lazy-import patch below)"
...
add_yaml_include_or_fail "${WHEEL_SRC}/dependencies.yaml" "py_run_raft_dask" "depends_on_ucx_build" \
    "add libucx-cu13 as a runtime (not just build-time) dependency of raft-dask-rtx50xx"
...
if [[ -f "${RELEASE_NOTES}" ]]; then
    ...
    RELEASE_NOTES_ARG=(--notes "pylibraft-rtx50xx-cu${CUDA_VERSION_COMPACT:0:2} + libraft-rtx50xx-cu${CUDA_VERSION_COMPACT:0:2} + raft-dask-rtx50xx-cu${CUDA_VERSION_COMPACT:0:2} ${VERSION}+cu${CUDA_VERSION_COMPACT} wheels for x86_64 / CUDA ${CUDA_VERSION} / SM_${RTX50_CUDA_ARCH} (RTX 50xx / Blackwell), bundled with the shared librmm-cu13/rmm-cu13 $(basename "${LIBRMM_WHEEL}") build (no device code -- shared across GPU variants, see raft_wheel_librmm_shared.sh)")
fi
