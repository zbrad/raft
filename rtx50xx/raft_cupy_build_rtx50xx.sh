#!/bin/bash
# raft_cupy_build_rtx50xx.sh — Build a CUDA-arch-specific cupy wheel for RTX
# 50xx (SM_120 / Blackwell consumer / x86_64) and place it in dist/rtx50xx/.
# Moved from wsl/raft_cupy_build_rtx50xx.sh.
#
...
#   rtx40xx/raft_cupy_build_rtx40xx.sh  — RTX 40xx  (SM_89,   x86_64)
#
# ── NOTE: untested — run on an x86_64 host with RTX 50xx + CUDA installed ────
#
# ── Usage ─────────────────────────────────────────────────────────────────────
#   bash rtx50xx/raft_cupy_build_rtx50xx.sh      # default version + output dir
#   CUPY_VERSION=14.0.1 bash rtx50xx/...         # pin a specific cupy version
#   DIST_DIR=/path/to/out bash rtx50xx/...       # override output directory
#
...
CUDA_ARCH="${CUDA_ARCH:-${RTX50_CUDA_ARCH}}"
...
echo ""
echo "════════════════════════════════════════════════════════════════"
echo "  Building cupy-${CUPY_VERSION} for SM_${CUDA_ARCH} (RTX 50xx / Blackwell)"
