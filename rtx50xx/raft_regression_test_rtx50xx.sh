#!/bin/bash
# raft_regression_test_rtx50xx.sh — run the two targeted regression tests
# for RTX 50xx: UTILS_TEST (warpReduce ADL fix) and LINALG_TEST (laplacian
# NZType / SM_121a memory-corruption fix) -- the two known upstream bugs
# this fork carries fixes for. NOT a full test suite; see
# raft_full_test_rtx50xx.sh for that (runs every built gtest binary).
# Renamed from raft_test_rtx50xx.sh to make this scope explicit.
#
