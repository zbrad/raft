...
# raft 26.6 for aarch64-cu132-sm103

## Branch: zbrad/raft gb10

Custom release for NVIDIA DGX Spark with Grace CPU (aarch64) and Blackwell GPU (SM 103), CUDA 13.2.

### 🐛 Bug Fixes
* Fix type mismatch in sparse Laplacian computation (`laplacian.cuh`) — `marked_diagonal` was allocated as `device_vector<int>` but consumed by `thrust::exclusive_scan` expecting `device_vector<NZType>` output. On SM_121 (Grace Blackwell / GB10) with CCCL 3.4.0, the "warpspeed scan" optimization activates this code path and caused writes to incorrect memory addresses, corrupting the CUDA context. Changed `marked_diagonal` to `device_vector<NZType>` to ensure type consistency. Fixes COO and CSR sparse Laplacian, normalized Laplacian, and 64-bit index (`NZType=long`) support.
