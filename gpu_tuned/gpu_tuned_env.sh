#!/bin/bash
# gpu_tuned_env.sh <variant> — detect CUDA installation and export common
...
# `source gpu_tuned/gpu_tuned_env.sh <variant>`). Formerly three
...
#                          — from gpu_tuned/devices/<variant>.conf, re-exported here for convenience
#
# Also defines gpu_tuned_verify_arch() -- see below.

GPU_TUNED_ARG_VARIANT="$1"
if [[ -z "${GPU_TUNED_ARG_VARIANT}" ]]; then
    echo "ERROR: gpu_tuned_env.sh requires a variant argument (gb10/rtx40xx/rtx50xx)" >&2
...
if [[ "$(uname -m)" != "${GPU_TUNED_PLATFORM}" ]]; then
    echo "ERROR: gpu_tuned_env.sh: expected platform '${GPU_TUNED_PLATFORM}' for" \
