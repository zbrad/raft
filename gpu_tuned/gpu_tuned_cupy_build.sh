#!/bin/bash
# gpu_tuned_cupy_build.sh <variant> — Build a CUDA-arch-specific cupy
...
#
# ── Usage ─────────────────────────────────────────────────────────────────────
#   bash gpu_tuned/gpu_tuned_cupy_build.sh rtx50xx      # default version + output dir
#   CUPY_VERSION=14.0.1 bash gpu_tuned/... rtx50xx      # pin a specific cupy version
#   DIST_DIR=/path/to/out bash gpu_tuned/... rtx50xx    # override output directory
#   CUPY_NUM_BUILD_JOBS=16 bash gpu_tuned/... rtx50xx   # parallel C++ compile jobs
#
...
# see gpu_tuned/devices/*.conf for why each variant needs what it needs.
...
echo "        or:  bash ${GPU_TUNED_VARIANT}/raft_wheel_${GPU_TUNED_VARIANT}.sh"
echo ""
echo "Done."
