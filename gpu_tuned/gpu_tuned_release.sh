#!/bin/bash
# gpu_tuned_release.sh <variant> — publish the given GPU variant's C++
# tarball as a GitHub release. Shared implementation behind every
# gb10/rtx40xx/rtx50xx raft_release_<variant>.sh wrapper.
...
RELEASE_TAG="v${SHORT_VER}.0-${GPU_TUNED_PLATFORM}-cuda${CUDA_VERSION_COMPACT}-${GPU_TUNED_VARIANT}"
...
# -- gpu_tuned_wheel.sh derives this identically, independently (they're
...
RELEASE_NOTES_FILE="RELEASE_NOTES_${SHORT_VER}_${GPU_TUNED_VARIANT}_cu${CUDA_VERSION_COMPACT}.md"

gh release create "${RELEASE_TAG}" --repo zbrad/raft \
  "${PKG_NAME}.tar.bz2#RAFT ${SHORT_VER} ${GPU_TUNED_PLATFORM} CUDA ${CUDA_VERSION} binary package (${GPU_TUNED_DEVICE_LABEL})" \
  "CHECKSUMS_${GPU_TUNED_VARIANT}#CHECKSUMS_${GPU_TUNED_VARIANT}" \
