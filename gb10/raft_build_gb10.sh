...
echo "raft-gb10 build: https://github.com/zbrad/raft @ ${GIT_COMMIT}, CUDA ${CUDA_VERSION}, sm_${GB10_CUDA_ARCH}, built ${BUILD_TIMESTAMP}" > "${BUILD_INFO_FILE}"
objcopy --add-section .raft_build_info="${BUILD_INFO_FILE}" "${PROJECT_ROOT}/cpp/build-${ARCH}/libraft_gb10.so"
