...
RELEASE_TITLE="RAFT ${SHORT_VER} — ${ARCH} / CUDA ${CUDA_VERSION} / SM_121a (GB10 / DGX Spark) wheels"
...
cp /tmp/raft-gb10-install/lib/libraft.so "${LIBRAFT_LIB64}/libraft.so"
rm -rf /tmp/raft-gb10-install

# Patch: rename to libraft-gb10, embed CUDA minor version in VERSION,
# disable GIT_COMMIT path lookup
sed -i \
...
echo "Building libraft-gb10 wheel v${VERSION}+cu${CUDA_VERSION_COMPACT} for ${ARCH} (bundles libraft.so)..."
...
# Restore libraft pyproject.toml, VERSION, and lib64 before pylibraft build
sed -i \
    -e "s/^name = \"libraft-gb10\"/name = \"libraft\"/" \
    -e "/^commit-files = \[\]/d" \
    "${LIBRAFT_PYPROJECT}"
...
echo "Building pylibraft-gb10 wheel v${VERSION}+cu${CUDA_VERSION_COMPACT} for ${ARCH} (SM_121a only)..."
SKBUILD_CMAKE_ARGS="-Draft_ROOT=${INSTALL_DIR};-DCMAKE_PREFIX_PATH=${INSTALL_DIR};-DCMAKE_CUDA_ARCHITECTURES=121a" \
    pip wheel \
        --no-deps \
        --no-build-isolation \
        --wheel-dir "${DIST_DIR}" \
...
if [[ -f "${RELEASE_NOTES}" ]]; then
    ...
    RELEASE_NOTES_ARG=(--notes "pylibraft-gb10-cu${CUDA_VERSION_COMPACT:0:2} + libraft-gb10-cu${CUDA_VERSION_COMPACT:0:2} ${VERSION}+cu${CUDA_VERSION_COMPACT} wheels for ${ARCH} / CUDA ${CUDA_VERSION} / SM_121a (GB10 / DGX Spark)")
fi
