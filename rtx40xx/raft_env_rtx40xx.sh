#!/bin/bash
# raft_env_rtx40xx.sh — detect CUDA installation and export common env vars.
...
#   RTX40_CUDA_ARCH — the one true CUDA arch string for RTX 40xx builds: "89"
#
# ── Rule: RTX 40xx must target bare sm_89, not a family-specific "a" variant
# RTX 40xx (Ada Lovelace) is consumer silicon -- there is no family-specific
# ("accelerated") ISA extension exposed on it, unlike datacenter-class chips
# (e.g. GB10 needs sm_121a -- see gb10/raft_env_gb10.sh). "89" covers the
# whole 40-series (4060/4070/4080/4090 all share sm_89) since none of those
# SKUs differ in compute capability -- only in core/memory counts. Every
# rtx40xx/* script must derive its CUDA arch from RTX40_CUDA_ARCH rather than
# hardcoding "89" so this can't drift.
RTX40_CUDA_ARCH="89"
export RTX40_CUDA_ARCH

# verify_rtx40xx_arch <path-to-.so> — assert a compiled library's embedded
# cubins are exactly sm_89 (via cuobjdump), catching a build silently
# produced against the wrong CMAKE_CUDA_ARCHITECTURES.
verify_rtx40xx_arch() {
    local so_file="$1"
    if [[ ! -f "${so_file}" ]]; then
        echo "ERROR: verify_rtx40xx_arch: no such file: ${so_file}" >&2
        return 1
    fi
    local found
    found="$(cuobjdump --list-elf "${so_file}" 2>/dev/null | grep -oE 'sm_[0-9]+a?' | sort -u)"
    if [[ "${found}" != "sm_${RTX40_CUDA_ARCH}" ]]; then
        echo "ERROR: ${so_file} is not built for sm_${RTX40_CUDA_ARCH} (found: ${found:-none})" >&2
        return 1
    fi
    echo "Verified: ${so_file} is built for sm_${RTX40_CUDA_ARCH}"
}
#
