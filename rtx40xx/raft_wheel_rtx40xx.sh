...
patch_line_or_fail "${PYLIBRAFT_STAGED}/pyproject.toml" \
    '^name = "pylibraft"' 'name = "pylibraft-rtx40xx"' "pylibraft package name"
patch_line_or_fail "${PYLIBRAFT_STAGED}/pyproject.toml" \
...
patch_line_or_fail "${WHEEL_SRC}/dependencies.yaml" \
    "- libraft-cu${CUDA_VERSION_COMPACT:0:2}==" "- libraft-rtx40xx-cu${CUDA_VERSION_COMPACT:0:2}==" \
...
# ── 3. Build raft-dask-rtx40xx-cuXX ───────────────────────────────────────────
...
patch_line_or_fail "${RAFT_DASK_STAGED}/pyproject.toml" \
    '^name = "raft-dask"' 'name = "raft-dask-rtx40xx"' "raft-dask package name"
patch_line_or_fail "${RAFT_DASK_STAGED}/pyproject.toml" \
    '^matrix-entry = .*' 'matrix-entry = "cuda_suffixed=true"\ncommit-files = []' \
